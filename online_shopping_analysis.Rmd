...
The Online Shoppers Purchasing Intention Dataset from the UCI Machine Learning Repository, has 12,330 data points and consists of both numerical and categorical attributes. The models I used here include logistical regression, support vector machines, k-nearest-neighbour, which helped to detect online shoppers purchasing patterns and forcast their intention. The outline of the analysis is as follows:
...
Only ten months of data are included in the data set, no January and April data. March, May, November and December were the four months with significant online shopping performance (both browsing and purchasing). The holiday seasons account for shopping intention which makes sense, but why March and May, particularly, the performance in May even better than November. It was not explained. In a business context, we need to investigate that:
...
Use all features to build the first logistic model and then select six significant ones based on P-value to build a simpler model and compare their performance.
...
```{r}
data <- data %>% mutate(Weekend = if_else(Weekend == 'FALSE', 0, 1))
...
It seems redundant to split data again. However, the above categorical data has been mutated, so it is a necessary step. The set seed ensures that the training, validation and test sets are corresponding to the split data for logistic regression models. Thus we are able to compare the models' accuracy and do the selection using the validation set.
...
SVM has 84.69% of accuracy on the validation set.
...
validation <- predict(svm, validation_set[, 1:17])
validation_acc = sum(validation == validation_set[, 18]) / nrow(validation_set)
validation_acc
```

#4.2 K-nearest Neighbor Model (KNN)

As KNN is not model based and there is no training or validation step, I would like to use all the data in KNN to test its performane. However, the model ran really slow when there were a lot of observations in this case. I was not able to generate the output here due to time constraints, so just attaching the coding here. 
...
Instead, given the data including web browsing metrics, k-means clustering could be used to subset online shoppers into similar groups. If more customer profile data was given, we would be able to find out the common shopping behavior or characteristics of customers in a cluster, which may help promotion decision-making. 
...
According to the prediction accuracy on the validation set and the algorithm efficiency, the logistic regression model with six features stands out (88.43% accuracy on the validation set), in which $ExitRates$, $PageValues$ and $Month$ are significant to the shopping intention. In other words, the percentage that were the last in the session for all pageviews to the page, the average value for a web page that a user visited before completing an e-commerce transaction, and the shopping timing highly correlated with the purchase.

To answer the second question, we may use the test set to estimate the model's quality: the model has the accuracy of 87.68% for the prediction of an online purchase intention.
...
In a business context, it would be more complicated. Do we prefer to detect a less strong purchase intention and thus take actions such as emailing offers to motivate customers to buy? Or would we like to better target leads to increased sales due to cost constraints?
